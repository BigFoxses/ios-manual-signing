...
def private_sign_ipa_run_command(cmd, _stdin=None, _stdout=None, _stderr=None):
    try:
        ...
        if isinstance(cmd, list):
            cmd = cmd[0]
        message = "Invalid arguments when trying to run %s ErrorMessage: %s" % (
            cmd,
            error,
        )
        private_sign_ipa_die_with_error(message)
        ...
        with zipfile.ZipFile(output, "w") as zip_handler:
            for root, dirs, files in walk(realpath(folder)):
                ...
                for file_name in files:
                    ...
                    compression = (
                        self.files_dict[rel_path]
                        if rel_path in self.files_dict
...
def parse_args():
    ...
    parser.add_argument(
        "-o", "--output", help="Path of output file (file name included)", required=True
    )
    parser.add_argument(
        "-s", "--signer", help="String to use as signer identity", required=True
    )
...
class PrivateSignIpa(object):
    ...
    def validate_metadata(self, entitlements_path, executable_path):
        if self.sha1_fingerprint is None:
            return
        entitlements_metadata_path = entitlements_path_to_metadata_path(
            entitlements_path
        )
        ...
        with open(entitlements_metadata_path) as metadata_file:
            ...
            if (
                provisioning_profile_metadata
                and len(provisioning_profile_metadata) > 0
                and self.sha1_fingerprint not in provisioning_profile_metadata
            ):
                message = (
                    "The input certificate doesn't match the provisioning profile.\nValid certificates are:\n%s"
                    % (
                        "\n".join(
                            [
                                "Cert: [%s], with fingerprint: [%s]"
                                % (cert_name, fingerprint)
                                for (
                                    fingerprint,
                                    cert_name,
                                ) in provisioning_profile_metadata.items()
                            ]
                        ),
                    )
                )
        ...
        if self.signing_directory:
            private_sign_ipa_log(
                "Successfully matched provisioning profile with signing identity for executable %s"
    ...
    def sign_sub_folders(self, folder):
        ...
        for dir_name in [
            file_name for file_name in files_list if isdir(join(folder, file_name))
        ]:
            dir_path = join(folder, dir_name)
            total_files_signed += self.sign_sub_folders(dir_path)

        for file_name in [
            file_name for file_name in files_list if not isdir(join(folder, file_name))
        ]:
    ...
    def sign(self, target, entitlements):
        try:
            p = private_sign_ipa_run_command(
                self.generate_codesign_cmd(entitlements, target)
            )
            if self.signing_directory:
                private_sign_ipa_log(
                    "About to sign [%s] using codesign"
                    % (relpath(target, self.signing_directory),)
                )
            p.communicate()
            res = p.returncode
            if res != 0:
            ...
            private_sign_ipa_die_with_error(
                "Failed to execute codesign with error: " + str(e)
            )
...
def validate_cert(identity):
    ...
    if len(identity) == 40 and identity in keychain_certs_fingerprint_list:
        ...
        private_sign_ipa_log(
            "Successfully matched certificate SHA-1 fingerprint [%s] in keychain"
            % (signing_cert_fingerprint,)
        )
    else:
        matching_fingerprints_by_name = [
            fingerprint
            for fingerprint, name in keychain_certs.items()
            if name == identity
        ]
        ...
        private_sign_ipa_log(
            "Successfully matched signing identity [%s] to SHA-1 fingerprint [%s] in keychain"
...
def main():
    ...
    with private_sign_ipa_erased_temp_dir() as temp_dir:
        ...
        private_sign_ipa_log(
            "Signed ipa output file location:\n ===> %s <==="
            % (abspath(script_args.output))
        )
